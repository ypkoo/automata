...
def construct_hangul_automata():
    ...
    def output_k(prev_str, input):
        ...
        if input in vowels:
            return prev_str[0:-1] + make_char(last_char, input)
        elif input in consonants:
            return prev_str[0:-2] + make_char(cho, jung, last_char) + input
    ...
    def output_l(prev_str, input):
        ...
        if input in consonants:
            if jong != ' ':
                jong = merge_jong(jong, last_char)
                return prev_str[0:-2] + make_char(cho, jung, jong) + input
            else:
                return prev_str[0:-2] + make_char(cho, jung, last_char) + input
